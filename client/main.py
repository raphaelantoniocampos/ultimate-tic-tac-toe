...
try:
    ...
    try:
        ...
        class DummyLogic:
            def generate_board(self): return []
        game = DummyLogic()
...
try:
    ...
    # Fallbacks
    BOARD_IMG = pygame.Surface((772, 772)); BOARD_IMG.fill((200, 200, 200))
...
def get_player_image(player):
    if player == "X": return X_IMG
    if player == "O": return O_IMG
    if player == "D": return MINI_DRAW_IMG
    return None


def get_winner_image(winner_player):
    if winner_player == "X": return WINNING_X_IMG
    if winner_player == "O": return WINNING_O_IMG
    if winner_player == "D": return DRAW_IMG
    return None


async def connect_and_listen():
    global game_status, my_player, board, to_move, winner, error, ws_client
    if not ws_client: return
    try:
        ...
        for j in range(3):
            ...
            for mi in range(3):
                for mj in range(3):
                    ...
                    if isinstance(mark, str) and graphical_board[i][j][mi][mj][0] is None:
                        img = get_player_image(mark)
                        px = center_x + (mj - 1) * mini_spacing
                        py = center_y + (mi - 1) * mini_spacing
                        graphical_board[i][j][mi][mj] = [img, img.get_rect(center=(px, py))]
            if len(board[i][j]) > 3:
...
def draw_menu():
    ...
    create_btn = pygame.Rect(0, 0, 200, 50); create_btn.center = (WIDTH // 2, HEIGHT_SLICE * 7)
    pygame.draw.rect(SCREEN, FILL_COLOR, create_btn, border_radius=5)
    SCREEN.blit(game_state_font.render("Create Game", True, BG_COLOR), game_state_font.render("Create Game", True, BG_COLOR).get_rect(center=create_btn.center))
    input_box = pygame.Rect(0, 0, 300, 50); input_box.center = (WIDTH // 2, HEIGHT_SLICE * 9)
    pygame.draw.rect(SCREEN, BG_COLOR, input_box, border_radius=5); pygame.draw.rect(SCREEN, TEXT_COLOR, input_box, 2, border_radius=5)
    ...
    join_btn = pygame.Rect(0, 0, 200, 50); join_btn.center = (WIDTH // 2, HEIGHT_SLICE * 11)
    pygame.draw.rect(SCREEN, FILL_COLOR, join_btn, border_radius=5)
    SCREEN.blit(game_state_font.render("Join Game", True, BG_COLOR), game_state_font.render("Join Game", True, BG_COLOR).get_rect(center=join_btn.center))
    if error:
    ...
    quit_btn = pygame.Rect(0, 0, 200, 50); quit_btn.center = (WIDTH // 2, HEIGHT_SLICE * 15)
    pygame.draw.rect(SCREEN, FILL_COLOR, quit_btn, border_radius=5)
    SCREEN.blit(game_state_font.render("Quit", True, BG_COLOR), game_state_font.render("Quit", True, BG_COLOR).get_rect(center=quit_btn.center))
...
def draw_waiting():
    ...
    cancel_btn = pygame.Rect(0, 0, 200, 50); cancel_btn.center = (WIDTH // 2, HEIGHT_SLICE * 11)
    pygame.draw.rect(SCREEN, FILL_COLOR, cancel_btn, border_radius=5)
    SCREEN.blit(game_state_font.render("Cancel", True, BG_COLOR), game_state_font.render("Cancel", True, BG_COLOR).get_rect(center=cancel_btn.center))
    return cancel_btn
...
def draw_game_window():
        ...
        for j in range(3):
            for mi in range(3):
                for mj in range(3):
                    ...
                    if cell and cell[0]: SCREEN.blit(cell[0], cell[1])
    ...
    id_text = game_state_font.render(f"ID: {game_id}", True, TEXT_COLOR); id_rect = id_text.get_rect(topleft=(20, HEIGHT_SLICE * 1))
    id_rect.centery = HEIGHT_SLICE * 1; SCREEN.blit(id_text, id_rect)
    if my_player in ["X", "O"]:
        you_img, end_x, y_pos = (X_IMG if my_player == "X" else O_IMG), WIDTH - 20, HEIGHT_SLICE * 1
        txt_surf = game_state_font.render("You: ", True, TEXT_COLOR); txt_rect = txt_surf.get_rect()
        img_rect = you_img.get_rect(); img_rect.midright = (end_x, y_pos); txt_rect.midright = (img_rect.left - 10, y_pos)
        SCREEN.blit(txt_surf, txt_rect); SCREEN.blit(you_img, img_rect)
    elif my_player == "SPECTATOR":
        spec_text = game_state_font.render("SPECTATING", True, TEXT_COLOR)
        SCREEN.blit(spec_text, spec_text.get_rect(midright=(WIDTH - 20, HEIGHT_SLICE * 1)))
    status_text_str, status_img = "", None
    if game_finished:
        status_text_str = "Winner: " if winner != "D" else "Game Over: DRAW"
        if winner != "D": status_img = X_IMG if winner == "X" else O_IMG
    ...
    if status_img:
        txt_surf = game_state_font.render(status_text_str, True, TEXT_COLOR); txt_rect = txt_surf.get_rect()
        total_width = txt_rect.width + status_img.get_width() + 10; start_x = bottom_center[0] - (total_width // 2)
        txt_rect.topleft = (start_x, bottom_center[1] - txt_rect.height // 2); SCREEN.blit(txt_surf, txt_rect)
        img_rect = status_img.get_rect(); img_rect.midleft = (txt_rect.right + 10, txt_rect.centery); SCREEN.blit(status_img, img_rect)
    else:
        status = game_state_font.render(status_text_str, True, TEXT_COLOR); SCREEN.blit(status, status.get_rect(center=bottom_center))
    exit_btn = pygame.Rect(0, 0, 120, 40); exit_btn.center = (WIDTH - 80, HEIGHT_SLICE * 17)
    pygame.draw.rect(SCREEN, FILL_COLOR, exit_btn, border_radius=5)
    SCREEN.blit(game_state_font.render("Exit", True, BG_COLOR), game_state_font.render("Exit", True, BG_COLOR).get_rect(center=exit_btn.center))
    return exit_btn
...
async def perform_handshake(command, payload=None):
    ...
    try:
        ...
        ws_client = WSClient(); await ws_client.connect(uri)
    except Exception as e:
        print(f"DEBUG: Handshake connection error: {e}"); error = f"Conn failed: {e}"; return False
    if command == "CREATE":
        await ws_client.send(pickle.dumps(("CREATE",)))
        try:
            message = await ws_client.receive()
            resp = pickle.loads(message); game_id, my_player, game_status = resp[1], resp[2], "WAITING"; return True
        except Exception as e: error = f"Handshake recv err: {e}"
    elif command == "JOIN":
        await ws_client.send(pickle.dumps(("JOIN", payload)))
        try:
            message = await ws_client.receive()
            resp = pickle.loads(message)
            if resp[0] == "ERROR": error = resp[1]; return False
            elif resp[0] == "JOINED": my_player, game_id, game_status = resp[1], payload, "GAME"; return True
        except Exception as e: error = f"Handshake recv err: {e}"
    if ws_client: await ws_client.close(); ws_client = None
    return False
...
def reset_game():
    ...
    board = game.generate_board()
    graphical_board = [[[[[None, None] for _ in range(3)] for _ in range(3)] for _ in range(3)] for _ in range(3)]
    to_move, my_player, game_finished, game_status, winner, game_id, error, input_text = ["X", None], None, False, "MENU", None, "", "", ""
    if ws_client: asyncio.create_task(ws_client.close()); ws_client = None
            ...
            for event in events:
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if create_btn.collidepoint(event.pos):
                        if await perform_handshake("CREATE"): asyncio.create_task(connect_and_listen())
                    elif join_btn.collidepoint(event.pos):
                        if input_text:
                            if await perform_handshake("JOIN", input_text): asyncio.create_task(connect_and_listen())
                    elif quit_btn.collidepoint(event.pos): running = False
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_BACKSPACE: input_text = input_text[:-1]
                    elif len(input_text) < 10: input_text += event.unicode
        elif game_status == "WAITING":
            cancel_btn = draw_waiting()
            for event in events:
                if event.type == pygame.MOUSEBUTTONDOWN and cancel_btn.collidepoint(event.pos): reset_game()
        else:
            exit_btn = draw_game_window()
            for event in events:
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if exit_btn.collidepoint(event.pos): reset_game(); continue
                    if game_status == "GAME" and my_player == to_move[0] and my_player != "SPECTATOR":
                        x, y = pygame.mouse.get_pos(); col, row = (x - BOARD_OFFSET) // CELL_SIZE, (y - BOARD_OFFSET) // CELL_SIZE
                        if 0 <= col <= 2 and 0 <= row <= 2:
                            rx, ry = (x - BOARD_OFFSET) % 230, (y - BOARD_OFFSET) % 230
                            mc, mr = rx // MINI_CELL_SIZE, ry // MINI_CELL_SIZE
                            if mc <= 2 and mr <= 2:
                                try:
                                    if ws_client: await ws_client.send(pickle.dumps((row, col, mr, mc)))
                                except Exception as e: print(f"Error sending move: {e}")
        pygame.display.update(); await asyncio.sleep(0); clock.tick(30)
    if ws_client: await ws_client.close()
    pygame.quit()
